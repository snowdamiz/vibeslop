defmodule Backend.Content do
  ...
  def list_unified_feed(opts \\ []) do
    ...
    project_query = case feed_type do
      "following" ->
        # TODO: Filter by followed users
        project_query
      _ ->
    ...
    reposts_query = case feed_type do
      "following" ->
        # TODO: Filter by followed users
        reposts_query
      _ ->
    ...
    feed_items = (posts ++ projects ++ reposts)
  ...
  defp add_reposts_count(feed_items) do
    Enum.map(feed_items, fn item ->
      {item_type, item_id} = case item.type do
  ...
  def create_project(user_id, attrs) do
    Repo.transaction(fn ->
      ...
      prompts = Map.get(attrs, "prompts", []) || Map.get(attrs, :prompts, []) || []
      ...
      # Add optional fields
      project_attrs = if live_url, do: Map.put(project_attrs, "live_url", live_url), else: project_attrs
      project_attrs = if github_url, do: Map.put(project_attrs, "github_url", github_url), else: project_attrs

      # Create project
      project = case Repo.insert(Project.changeset(%Project{}, project_attrs)) do
        {:ok, project} -> project
      ...
      |> Enum.map(fn tool_name ->
        ...
        case Backend.Catalog.get_ai_tool_by_slug(slug) do
          nil ->
            # Create new tool
            {:ok, tool} = Repo.insert(%Backend.Catalog.AiTool{
              name: tool_name,
              slug: slug
            })
            tool
      ...
      |> Enum.map(fn stack_name ->
        ...
        case Backend.Catalog.get_tech_stack_by_slug(slug) do
          nil ->
            # Create new stack
            {:ok, stack} = Repo.insert(%Backend.Catalog.TechStack{
              name: stack_name,
              slug: slug,
              category: "other"
            })
      ...
      prompts
      |> Enum.with_index()
      |> Enum.each(fn {prompt_data, index} ->
        prompt_attrs = %{
          title: Map.get(prompt_data, "title") || Map.get(prompt_data, :title),
          description: Map.get(prompt_data, "description") || Map.get(prompt_data, :description),
          code: Map.get(prompt_data, "code") || Map.get(prompt_data, :code),
          position: index,
          project_id: project.id
        }
        %Backend.Content.ProjectPrompt{}
        |> Backend.Content.ProjectPrompt.changeset(prompt_attrs)
        |> Repo.insert!()
      end)
  ...
  def list_comments(commentable_type, commentable_id, opts \\ []) do
    ...
    Enum.map(comments, fn comment_data ->
      ...
      is_liked = if current_user_id do
